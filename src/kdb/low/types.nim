...
{.passC: "-DKXVER=3".}
{.passC: "-I" & currentSourcePath.parentDir().parentDir().parentDir().}
{.link: currentSourcePath.parentDir().parentDir().parentDir().parentDir() & "/c.o".}
{.compile: "k.c".}
...
proc `=destroy`*(x: var K) =
  if x.k != nil:
    ...
    # echo "destroy K: ", x.k.kind, " rc = ", rc, " addr = " # , repr(x.k)
    r0(x.k)
